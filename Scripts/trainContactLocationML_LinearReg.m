...
function [xModel, yModel, zModel] = trainContactLocationML_LinearReg(touchData)
    ...
    % Create the x, y, and z model estimates
    xModel = fitlm(sensorStateMat, touchPosMat(:,1));
    yModel = fitlm(sensorStateMat, touchPosMat(:,2));
    zModel = fitlm(sensorStateMat, touchPosMat(:,3));
